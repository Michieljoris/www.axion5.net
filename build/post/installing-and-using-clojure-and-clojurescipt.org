...
Setting optimizations to something other than none slows down compilation greatly. Also make sure =source-map= is set to true in this case, not a string. Compilation is quite fast with recompiling in auto mode taking a fraction of a second when a file changes. 
...
: lein repl
It is also possible to connect the ClojureScript repl to a Rhino JavaScript environment. The procedure is ery 
...
#+BEGIN_SRC clojure
  (do (require '[cljs.repl :as repl])
      ...
      (repl/repl env)_ ;; start the REPL
#+END_SRC
...
This will open a cljs prompt, but it will not be responsive. What you've done in face is start a server in the Clojure runtime that listens on port 8090. You will need a JavaScript runtime to connect to this and start interacting with the repl and evaluate JavaScript code.  

Create a webpage, for example, index.html that contains the following html:
#+BEGIN_SRC html
<html>
  <head>
    <meta charset="UTF-8">
...
View the page in a browser. It includes and loads the JavaScript compiled from the following ClojureScript code. In our case the compiled JavaScript code is in "resources/public/js/main.js". It is compiled from the ClojureScript source in src-cljs/sample-app/core.cljs:
#+BEGIN_SRC clojure
...
Now the ClojureScript repl should be responsive and able to evaluate ClojureScript code in the browser's JavaScript environment. Try (. js/console (log "Hello!!!"))
...
Ignore the suggestion in the piggieback docs to use [[https://github.com/cemerick/austin][Austin]] instead. It does build on piggieback and adds features but but also ties in a Clojure server. It's easier to just start with plain piggieback first. Austin hopelessly confused me at first.
...
(ws-repl/connect "ws://localhost:8090")
